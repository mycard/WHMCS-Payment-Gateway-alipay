...
class Alipay {
    ...
    var $alipay_gateway        = 'https://pay.blockfightroushan.com/gateway.do?';
    ...
    function signParameters ($params) {
        ...
        */
        function buildRequestFormHTML($params, $method = 'post', $target = '_self') {
            ...
            "form_target" => $target,
            "form_method" => $method,
            "form_action" => $this->alipay_gateway."_input_charset=".trim(strtolower($this->config['input_charset'])),
            "form_charset" => $this->config['input_charset']
            ];
            foreach ($params as $key => $value) {
                $skin_vars["form_params"] .= "<input type='hidden' name='$key' value='$value'/>";
            }
            $skin_raw = file_get_contents(__DIR__ . "/../skin/default/pay_button.tpl");
            foreach ($skin_vars as $keyword => $skin_var){
                $skin_raw = str_replace('{$'.$keyword.'}',$skin_var,$skin_raw);
            }
            
            return $skin_raw;
        }
        
        function buildRequestQr($params, $method = 'post', $target = '_self') {
            $params = $this->buildSignedParameters($params);
            ...
            $skin_raw = file_get_contents(__DIR__ . "/../skin/default/qr_code.tpl");
            foreach ($skin_vars as $keyword => $skin_var){
                $skin_raw = str_replace('{$'.$keyword.'}',$skin_var,$skin_raw);
            }
